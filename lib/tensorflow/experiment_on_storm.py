...
def tune_RDAE(x, lam_list = [50], learn_rates = [0.1], inner = 50, outter = 5):
    ...
    for lam in lam_list:
        for rate in learn_rates:
            rae = RDAE.RDAE(sess = sess, seeds = 1.0, lambda_ = lam, layers_sizes =[784,400])
            L, S = rae.fit(x ,sess = sess, learning_rate = rate, batch_size = 40, inner_iteration = inner, iteration=outter, verbose=True)
