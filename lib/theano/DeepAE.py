...
class DeepAE(object):
    ...
    def getcost(self,x, learning_rate, cost_function=r'logloss'):
        ## forward computation
        ...
        if cost_function == 'euclidean':
            cost =  T.sum((x - output)**2)
        if cost_function == 'logloss':
            #L = - T.sum(self.x * T.log(z) + (1 - self.x) * T.log(1 - z), axis=1)
            cost =  T.mean( - T.sum(x * T.log(output) + (1-x) * T.log(1 - output), axis=1))
        gparams = T.grad(cost, self.params)
        updates = [(param , param - learning_rate * gparam) for param,gparam in zip(self.params , gparams)]
        return cost,updates
    def fit(self, X , iterations=15, batch_size = 10 ,learning_rate=0.15,errors=[], verbose=False,cost_function='logloss'):
        index = T.lscalar()
        x = T.matrix('x')
        n_train_batches = X.shape[0] / batch_size
        ##forward compute the cost
        cost,updates = self.getcost(x,learning_rate=learning_rate,cost_function=cost_function)
...
def testDAE(size=(100,10)):
    ...
    batch_size = 20
    learning_rate=0.05
    dae.fit(X = data,iterations=iterations, batch_size = batch_size,learning_rate=learning_rate,)
    print np.sum((dae.getRecon(X = data)-data)**2),np.sum((data)**2)
def testDAE2():
