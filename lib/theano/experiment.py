...
import ImShow as I
import numpy as np
import DeepRAE as DRAE
import DeepAE as DAE
import os
def compareDRAEandDAE(X, inputsize, hidden_layers_sizes, seed_num, learning_rate, lamda, batch_size, corruption_level, folder):
    ...
    drae = DRAE.DRAE3(hidden_layers_sizes = hidden_layers_sizes, seeds = seed_num ,lambda_=lamda, error = 0.001,
                corruption_level = corruption_level)
    L , S = drae.fit(X,learning_rate=learning_rate,inner_iteration = 5, iteration = 40,verbose = True,batch_size = batch_size)
    ...
    dae = DAE.DeepAE(visible_size=inputsize[0]*inputsize[1], hidden_layers_sizes=hidden_layers_sizes,seed_num=seed_num,corruption_level=corruption_level)
    error = []
    iteration = 5 * 40
    dae.fit(X = X,iterations= iteration, batch_size = batch_size,learning_rate=learning_rate,errors=error,verbose=True)
...
if __name__ =="__main__":
    ...
    #lamda = 10.0
    lam_list = [10.,20.,50.,70.,100.]
    for lamda in lam_list:
        folder = r"lam" + str(lamda)
        compareDRAEandDAE(X = X, inputsize = inputsize, hidden_layers_sizes = hidden_layers_sizes,
