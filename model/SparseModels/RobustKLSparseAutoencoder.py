...
import SparseKLAutoencoder as sdae
...
class Robust_KL_SparseAutoencder():
    ...
    """
    def __init__(self, sess, layers_sizes, sparsities, sparse_ratios,lambda_=1.0, error = 1.0e-5):
        ...
        assert len(sparsities) == len(sparse_ratios)
        self.SAE = sdae.Sparse_KL_Deep_Autoencoder( sess = sess, input_dim_list = self.layers_sizes,
                                                    sparsities = self.sparcities, sparse_ratios = self.sparse_ratios)
...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        rsae = Robust_KL_SparseAutoencder(sess = sess, lambda_= 4000, layers_sizes=[784,784,784,784],
                                            sparsities=[0.5,0.5,0.5], sparse_ratios= [0.2,0.2,0.2])
