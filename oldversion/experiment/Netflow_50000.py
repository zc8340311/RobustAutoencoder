...
def compare_frame():
    ...
    X = np.load(r"/home/zc8304/Documents/Netflow_50000.npk")
    inner = 120
    outer = 10

    #lamda_list = [0.00005,0.0005,0.005,0.05,0.5,5.0,50.0]
    lamda_list = np.arange(0.00012,0.0002,0.00001)
    layers = [X.shape[1], int(X.shape[1]/2.), int(X.shape[1]/4.)]

    for lam in lamda_list:
        folder = "lam" + str(lam)

        l21RDAE_compressFeature(X = X, layers=layers, lamda = lam, folder = folder, learning_rate = 0.005, inner = inner, outer = outer, batch_size = 1000,inputsize = (63,72))
